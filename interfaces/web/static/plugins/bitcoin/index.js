...
function injectStyles() {
  ...
  style.textContent = `
    ...
    .btc-security-banner .btc-banner-title {
      font-size: 14px;
      font-weight: 700;
      color: #ff6b35;
      margin-bottom: 8px;
    ...
    .btc-security-banner ul {
      margin: 6px 0;
      padding-left: 18px;
    }

    .btc-security-banner li { margin-bottom: 3px; }

    .btc-security-banner strong { color: #ff6b35; }

    .btc-security-banner .btc-banner-action {
      margin-top: 8px;
      padding: 8px 12px;
...
function renderWalletList(container) {
  ...
  container.innerHTML = `
    <div class="btc-form">
      <div class="btc-security-banner">
        <div class="btc-banner-title">\u26A0 Backup Your Keys</div>
        Private keys are <strong>AES-encrypted</strong> and stored in <code>~/.config/sapphire/credentials.json</code>.
        This file is <strong>not included in Sapphire backups</strong>.
        <ul>
          <li>Keys are <strong>permanently lost</strong> if: machine dies, OS reinstall, <code>~/.config/sapphire/</code> is deleted, or the salt file is removed</li>
          <li>Encryption is machine-bound &mdash; the key file cannot be decrypted on another machine</li>
          <li>Use <strong>Export Backup</strong> on each wallet to save a plaintext WIF you can restore anywhere</li>
        </ul>
        <div class="btc-banner-action">Export your wallet backups now. No backup = no recovery.</div>
      </div>
      <div class="btc-hint">
